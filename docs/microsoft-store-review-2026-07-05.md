...
`apps/desktop/src/main/index.ts:91-95` picks `pwsh` as the Windows default, which `packages/core/src/session.ts:27` resolves to `pwsh.exe`. PowerShell 7 is **not** preinstalled on Windows 10/11 — only Windows PowerShell 5.1 (`powershell.exe`) is. Certification testers run the app on a stock Windows image: they'll launch Awakon, hit "New session", and the PTY spawn fails (`session.ts:64` throws or the session dies instantly). The app's core function being broken on first run is a failure of Store Policy 10.1 (the app must be fully functional and testable).
...
**Fix:** probe for `pwsh.exe` on PATH and fall back to `powershell.exe`; show an actionable error when a user-selected shell isn't installed.
...
This is H1 of the general review: `packages/terminal-host/src/terminal-host.ts:95` loads `WebLinksAddon` with no handler, and no `setWindowOpenHandler`/`will-navigate` guard exists anywhere in `apps/desktop/src` (grep confirms zero occurrences). A tester who clicks a URL in terminal output gets a bare Electron window hosting an arbitrary remote site — no address bar, no browser controls, full preload bridge reachable. Manual certification testing treats in-app rendering of arbitrary web content in a non-browser app as a security failure (Policy 10.2), and this is exactly the kind of behavior the automated scan's dynamic analysis exercises.

**Fix (small):** deny-all `setWindowOpenHandler` on every `web-contents-created`, route link clicks through `shell.openExternal` after an `http:`/`https:` scheme check. Details in the general review, H1/M3.

### S2. End-of-life Electron with known Chromium CVEs
`apps/desktop/package.json:44` pins `electron: ^33.4.11`. Electron 33 (Chromium 130, released Oct 2024) left the supported window in 2025; as of mid-2026 it carries a long tail of publicly known, unpatched Chromium CVEs. Microsoft's binary scan fingerprints bundled runtimes, and a framework with known vulnerabilities is grounds for rejection under Policy 10.2 — and even if the automated scan passes, it is the first thing a manual security review checks in an Electron app. Compounding it, the Chromium sandbox is off in every renderer (`index.ts:461`, `view-manager.ts:76` — `sandbox: false`), so a renderer compromise via S1 + an old-Chromium exploit is unmitigated.

**Fix:** upgrade to a currently supported Electron major before submitting; re-enable the sandbox (general review M1 — nothing in the preload needs Node).

### S3. Simulated keystroke injection is on by default
General review M7: default settings ship `autoResume.enabled: true`, and any PTY output containing the detect phrase causes the app to *type into the terminal by itself* (`packages/core/src/session-manager.ts:57-63`). Two Store-specific angles:
...
**Fix:** ship disabled; enable only via explicit opt-in in Settings.

### S4. Static-analysis red flags in the Electron configuration
The automated scan and any manual review run standard Electron security tooling, which will flag, verbatim from this codebase:

| Flag | Evidence |
|---|---|
| `sandbox: false` on all renderers | `index.ts:461`, `view-manager.ts:76` |
| Fully generic IPC bridge (`send`/`on` for any channel, no allowlist) | `src/preload/index.ts:3-13` |
| No Content-Security-Policy on either page | `index.html`, `terminal-host.html` (no CSP meta, no header injection) |
| `shell.openExternal` accepts any URL scheme (`file:`, `smb:`, …) | `index.ts:227-232` + `packages/contracts/src/ipc.ts:203-205` (`z.string().url()` only) |
| Unscoped file read IPC (any `.md` anywhere on disk) | `apps/desktop/src/main/fs-handlers.ts:62-79` |

None of these alone is an automatic rejection, but together they profile as an unhardened app hosting untrusted content (terminal output + S1), which invites a manual security review. Fixes for all five are specified in the general review (M1, M2, M3, M8, L2).
...
`apps/desktop/src/main/app-menu.ts:54-55` ships `toggleDevTools` and `reload` unconditionally (general review L5). Not a hard policy violation, but certification testers press every menu item; DevTools opening in a shipped Store app reads as an unfinished product and occasionally draws a 10.1 "beta/test app" objection. Gate on `!app.isPackaged`.
...
- **The Linux `--no-sandbox` wrapper** (`afterPack.cjs`) exits early for non-Linux platforms and does not touch Windows builds (general review H4 still applies to Linux releases).
...
2. **B3** — `powershell.exe` fallback for the default shell + friendly missing-shell error.
3. **S1 + S4** — the Electron hardening batch from the general review (deny-all window-open handler, scheme allowlist, sandbox on, channel allowlist, CSP). One PR; it closes five scanner flags at once.
4. **S2** — Electron upgrade to a supported major.
5. **S3** — flip auto-resume to opt-in.

...
**Status (2026-07-06): test package prepared, awaiting a manual run.** Built the placeholder-identity MSIX (`dist:win:store`), generated a throwaway self-signed cert matching the appx's placeholder publisher, and signed the package — all done non-elevated. Installing it requires enabling Windows Developer Mode and trusting that cert at the machine level, both of which need admin rights this session doesn't have (UAC can't be scripted non-interactively). Two scripts are ready for you to run in an elevated PowerShell: `install-test-msix.ps1` (installs + prints the smoke-test checklist) and `cleanup-test-msix.ps1` (uninstalls, removes the cert, reverts Developer Mode) — see the scratchpad dir from this session, or ask for them to be copied somewhere durable.
